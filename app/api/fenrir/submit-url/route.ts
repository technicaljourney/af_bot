...
import { submitUrl, FenrirError } from "@/lib/fenrir";
import { getStoredToken } from "@/lib/authStore";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** Submit a private GitHub repo for fuzzing. */
export async function POST(req: NextRequest) {
  let body: { token?: string; repoUrl?: string; ref?: string; intakeChoice?: string };
  try {
    body = await req.json();
  } catch {
  ...
  try {
    const result = await submitUrl(token, {
      ...
      ref: body.ref,
      intakeChoice: body.intakeChoice,
    });
