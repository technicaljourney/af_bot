...
import { deleteSubmission, FeedbackError } from "@/lib/feedback";
...
export async function POST(req: NextRequest) {
  let body: { submissionId?: string; harborSubmissionId?: string; token?: string };
  try {
    body = await req.json();
  } catch {
  ...
  try {
    const result = await deleteSubmission(token, body.submissionId, body.harborSubmissionId);
